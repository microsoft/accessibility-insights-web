...
export const infoAndExamples = createWithTitle(
    ...
    ({ Markup, Link }) => (
        <React.Fragment>
            ...
            <p>
                Users can't access a web app's functionality if they can't access its interactive interface components. Many people,
                including those who are blind or who have low vision or hand tremors, rely on a keyboard (or keyboard alternate) to access
                those components.
            </p>
            <p>
                Keyboard users must be able to navigate using standard keyboard commands: <Markup.Term>Tab</Markup.Term> (which moves focus
                forward to the next component) and <Markup.Term>Shift+Tab</Markup.Term> (which moves focus backward to the previous
                component). They must be able to navigate between the focusable elements of a composite widget using{' '}
                <Markup.Term>arrow</Markup.Term> keys.
            </p>
            ...
            <ul>
                <li>
                    Links and native HTML form controls are in the tab sequence by default. If one of these elements can't be reached using
                    the <Markup.Term>Tab</Markup.Term> key, it's likely to have been removed from the tab sequence using{' '}
                ...
                <li>
                    <Markup.HyperLink href="https://www.w3.org/TR/wai-aria-practices/#aria_ex">Custom widgets</Markup.HyperLink> based on
                    HTML elements that are not natively focusable must be added to the tab sequence using{' '}
                ...
                <li>
                    ...
                    (such as a combo boxes, grids, menus, or tree views) have multiple focusable elements, only one of which should be
                    included in the tab sequence at any given time. The developer must ensure that a composite custom widget manages the
                    movement of focus between its focusable elements.
                </li>
            </ul>
            ...
            <Markup.PassFail
                failText={
                    <p>
                        This listbox can't be accessed via the <Markup.Term>Tab</Markup.Term> key because it isn't based on any elements
                        that are in the tab sequence by default.
                    </p>
                }
