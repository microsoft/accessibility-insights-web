...
export const infoAndExamples = create(({ Markup, Link }) => <>
  ...
  <p>Changing a component's settings must not trigger any unexpected change of context.</p>
    ...
    <p>
        A "change of context" is a major change in the content of a web page that might disorient users who aren't expecting the change. For example, opening a new window, moving focus to a different component, going to a new page, or significantly re-arranging the content of a page are all changes in context. <Markup.Term>Unexpected</Markup.Term> changes in context can be disorienting for anyone, but especially for people with visual disabilities, cognitive limitations, or motor impairments. Unless users are notified in advance, they will not expect changing a component's settings to trigger a change in context.
    </p>

    <h2>How to fix</h2>
    <p>
        Use a different trigger to initiate a change of context. For example, instead of initiating the change of context when a radio button is selected, provide a separate command button that triggers the change.
    </p>
