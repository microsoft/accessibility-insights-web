...
export const guidance = create(({ Markup, Link }) => (
    <>
        ...
        <p>
            ...
            it appears in the HTML), so it's important that the DOM order match
            the expected reading order. If they don't match, the content won't
            make sense to people who use assistive technologies.
        </p>

        <Markup.Columns>
            <Markup.Do>
                <h3>
                    When using CSS to position meaningful content, ensure it
                    still makes sense when CSS is disabled. (<Link.WCAG_1_3_1 />
                    )
                </h3>
                <ul>
                    <li>
                        If you use <Markup.Code>position:absolute</Markup.Code>,
                        be sure the DOM order matches the expected reading
                        order.{' '}
                    </li>
                    <li>
                        Avoid using <Markup.Code>float:right</Markup.Code>, as
                        it always creates a mismatch: elements displayed on the
                        right of the screen appear in the DOM before those on
                        the left.{' '}
                    </li>
                </ul>
                <h3>
                    When using layout tables to position content, ensure it
                    still makes sense when the table is linearized. (
                    <Link.WCAG_1_3_1 />)
                </h3>
                <ul>
                    <li>
                        Because assistive technologies read tables in DOM order,
                        tables are presented row-by-row, not column-by-column.
                    </li>
                </ul>
            </Markup.Do>
            <Markup.Dont>
                <h3>
                    Don't use white space characters to create the appearance of
                    columns.
                </h3>
                <ul>
                    <li>
                        Instead, use a layout table or CSS to display the
                        content in columns.
                    </li>
                </ul>
            </Markup.Dont>
