...
export const infoAndExamples = createWithTitle(
    ...
    ({ Markup, Link }) => (
        <>
            ...
            <p>
                Assistive technologies present content to users in DOM order (the order that content appears in the HTML). When CSS is used
                to modify only the <Markup.Emphasis>visual</Markup.Emphasis> order of content, assistive technologies might not be able to
                programmatically determine the expected reading order. As a result, people who use assistive technologies might receive
                content in an order that makes no sense.
            </p>
            <p>
                Also, some people with limited vision need to customize the styling of a web page in order to see its content. When they
                modify or disable CSS styling, positioned content might move to a location where its meaning is changed or lost.
            </p>
            <h2>How to fix</h2>
            <ul>
                <li>
            ...
            <Markup.PassFail
                failText={
                    <p>
                        A web page about U.S. presidents uses <Markup.Code>float:right</Markup.Code> to display an image of the home state
                        of each president. The presidents are listed in chronological order. In the{' '}
                        <Markup.Emphasis>visual</Markup.Emphasis> reading order, the content says, "George Washington was born on February
                        22, 1732, in Virginia. John Adams was born on October 30, 1735, in Massachusetts…" The DOM order is reversed, so
                        people who use assistive technology will encounter the content in an incorrect order: "Virginia. George Washington
                        was born on February 22, 1732 in Massachusetts. John Adams was born on October 30, 1735, in…"
                    </p>
                }
                failExample={`<style>
           [.right {float:right;}]
           ...
           ...`}
                passText={
                    <p>
                        The content is displayed in a layout table. The visual reading order matches the DOM order, so the president's name
