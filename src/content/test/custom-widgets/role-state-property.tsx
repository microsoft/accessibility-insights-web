...
export const infoAndExamples = create(({ Markup, Link }) => (
    <>
        ...
        <p>
            A custom widget must support the ARIA roles, states, and properties
            specified by its design pattern.
        </p>

        <h2>Why it matters</h2>
        <p>
            Making a custom widget involves modifying the role and/or function
            of one or more of the native HTML elements used as its building
            blocks. Additional coding is required to ensure that the widget
            provides the right information to browsers and assistive
            technologies.
        </p>
        ...
        <ol>
            ...
            <li>
                Make sure your custom widget follows the design pattern's
                specifications for "WAI-ARIA Roles, States, and Properties."
            </li>
        </ol>
        ...
        <Markup.PassFail
            failText={
                <p>
                    This list functions as a single-select listbox, but it does
                    not have the code necessary to communicate its roles and
            ...
            passText={
                <p>
                    ...
                    <Markup.HyperLink href="https://www.w3.org/TR/wai-aria-practices-1.1/#Listbox">
                        listbox
                    </Markup.HyperLink>{' '}
                    spec: (1) The <Markup.Code>{`<ul>`}</Markup.Code> that
                    contains all of the listbox options has{' '}
                    <Markup.Code>role="listbox"</Markup.Code>. (2) The{' '}
                    <Markup.Code>{`<ul>`}</Markup.Code> has an accessible name
                    through an <Markup.Code>aria-labelledby</Markup.Code>{' '}
                    attribute. (3) Each <Markup.Code>{`<li>`}</Markup.Code> has{' '}
                    <Markup.Code>role="option"</Markup.Code>. (4) The selected
                    option has <Markup.Code>aria-selected="true"</Markup.Code>.
                </p>
            }
