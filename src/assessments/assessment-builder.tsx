...
export class AssessmentBuilder {
    ...
    private static applyDefaultFunctions(requirement: Requirement): void {
        ...
        if (!requirement.getInstanceStatusColumns) {
            requirement.getInstanceStatusColumns =
                AssessmentBuilder.getInstanceStatusColumns;
        }

        if (!requirement.renderInstanceTableHeader) {
            requirement.renderInstanceTableHeader =
                AssessmentBuilder.renderInstanceTableHeader;
        }
    ...
    private static getInstanceStatus(
    ...
    private static renderRequirementDescription(
        requirementLink: RequirementLink,
    ): JSX.Element {
        return requirementLink.renderRequirementDescriptionWithIndex();
    }

    private static enableTest(
        scanData: ScanData,
        payload: AssessmentToggleActionPayload,
    ): void {
    ...
    private static disableTest(scanData: ScanData, requirement: string): void {
        ...
        scanAssessmentData.enabled = Object.keys(
            scanAssessmentData.stepStatus,
        ).some(key => scanAssessmentData.stepStatus[key] === true);
    }
    ...
    public static Manual(assessment: ManualAssessment): Assessment {
        ...
        assessment.requirementOrder =
            assessment.requirementOrder || RequirementComparer.byOrdinal;
        assessment.executeAssessmentScanPolicy =
            assessment.executeAssessmentScanPolicy ||
            AssessmentBuilder.nullScanPolicy;
        ...
        const visualizationConfiguration: AssesssmentVisualizationConfiguration = {
            ...
            getSwitchToTargetTabOnScan: this.getSwitchToTargetTabOnScan(
                requirements,
            ),
            getInstanceIdentiferGenerator: this.getInstanceIdentifier(
                requirements,
            ),
        };
    ...
    public static Assisted(assessment: AssistedAssessment): Assessment {
        ...
        const visualizationConfiguration: AssesssmentVisualizationConfiguration = {
            ...
            getDrawer: getDrawer,
            getNotificationMessage: getNotificationMessage,
            getSwitchToTargetTabOnScan: AssessmentBuilder.getSwitchToTargetTabOnScan(
                requirements,
            ),
            getInstanceIdentiferGenerator: AssessmentBuilder.getInstanceIdentifier(
                requirements,
            ),
    ...
    private static getSwitchToTargetTabOnScan(
        ...
        return (requirement: string): boolean => {
            ...
            if (
                requirementConfig == null ||
                requirementConfig.switchToTargetTabOnScan == null
            ) {
                return false;
            }
    ...
    private static getInstanceIdentifier(
        ...
        return (requirementKey: string) => {
            ...
            if (
                requirementConfig == null ||
                requirementConfig.generateInstanceIdentifier == null
            ) {
    ...
    private static buildRequirementReportDescription(
        requirements: Requirement[],
    ): void {
    ...
    private static removeLastDotFromDescription(children: any): any {
        if (Array.isArray(children)) {
            children[
                children.length - 1
            ] = AssessmentBuilder.removeLastDotFromDescription(
