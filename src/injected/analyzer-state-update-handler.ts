...
export class AnalyzerStateUpdateHandler {
    ...
    private terminateAnalyzers(
        ...
        types.forEach(visualizationType => {
            if (prevState != null) {
                ...
                const keys = this.getTestKeysFromConfiguration(
                    configuration,
                    currState,
                );
    ...
    private startAnalyzers(
        ...
        if (
            ...
            currState.injectingInProgress !== true
        ) {
            if (
                prevState == null ||
    ...
    private isTestTerminated(
        ...
        return (
            prevState != null && prevEnabled === true && currEnabled === false
        );
    }
    ...
    private getTestKeysFromConfiguration(
        ...
        if (this.isAssessment(config)) {
            const prevScanState = config.getStoreData(
                currState.tests,
