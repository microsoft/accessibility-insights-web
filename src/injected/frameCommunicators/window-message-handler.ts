...
export class WindowMessageHandler {
    ...
    private messageSubscribers: DictionaryStringTo<
        FrameMessageResponseCallback
    >;

    constructor(
        windowUtils: WindowUtils,
        windowMessageParser: WindowMessageMarshaller,
    ) {
    ...
    public post(
        ...
        const data = this.windowMessageParser.createMessage(
            command,
            message,
            responseId,
        );
    ...
    private windowMessageHandler = (e: MessageEvent): void => {
        ...
        const data: WindowMessage = this.windowMessageParser.parseMessage(
            e.data,
        );
        if (data == null) {
            return;
        ...
        const callback = this.callbacksForMessagesSentFromCurrentFrame[
            messageId
        ];

        try {
            if (callback) {
                this.processResponseFromPreviousRequest(
                    sourceWindow,
                    data,
                    callback,
                );
    ...
    private notifySubscriber(target: Window, data: WindowMessage): void {
        ...
        if (subscriber) {
            subscriber(
                ...
                data.error,
                target,
                this.createFrameResponderCallback(
                    target,
                    data.command,
                    data.messageId,
                ),
