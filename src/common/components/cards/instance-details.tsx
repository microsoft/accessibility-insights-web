...
export const InstanceDetails = NamedFC<InstanceDetailsProps>(
    'InstanceDetails',
    props => {
        const {
            result,
        ...
        const isHighlightSupported: boolean =
            deps.cardInteractionSupport.supportsHighlighting;
        ...
        const cardClickHandler = (event: React.SyntheticEvent): void => {
            if (isHighlightSupported) {
                deps.cardSelectionMessageCreator.toggleCardSelection(
                    result.ruleId,
                    result.uid,
                    event,
                );
