...
export class AssessmentStore extends BaseStoreImpl<AssessmentStoreData> {
    ...
    public generateDefaultState(
        persistedData: AssessmentStoreData = null,
    ): AssessmentStoreData {
        return this.initialAssessmentStoreDataGenerator.generateInitialState(
            persistedData,
        );
    }
    ...
    protected addActionListeners(): void {
        this.assessmentActions.getCurrentState.addListener(
        ...
        this.assessmentActions.resetAllAssessmentsData.addListener(
            this.onResetAllAssessmentsData,
        );
        this.assessmentActions.selectRequirement.addListener(
            this.onSelectTestStep,
        );
        this.assessmentActions.changeInstanceStatus.addListener(
            this.onChangeInstanceStatus,
        );
        this.assessmentActions.changeRequirementStatus.addListener(
            this.onChangeStepStatus,
        );
        this.assessmentActions.undoRequirementStatusChange.addListener(
            this.onUndoStepStatusChange,
        );
        ...
        this.assessmentActions.addFailureInstance.addListener(
            this.onAddFailureInstance,
        );
        this.assessmentActions.addResultDescription.addListener(
            this.onAddResultDescription,
        );
        this.assessmentActions.removeFailureInstance.addListener(
            this.onRemoveFailureInstance,
        );
        this.assessmentActions.editFailureInstance.addListener(
        ...
        this.assessmentActions.passUnmarkedInstance.addListener(
            this.onPassUnmarkedInstances,
        );
        this.assessmentActions.trackingCompleted.addListener(
            this.onTrackingCompleted,
        );
        this.assessmentActions.updateSelectedPivotChild.addListener(
            this.onUpdateSelectedTest,
        );
        this.assessmentActions.updateTargetTabId.addListener(
            this.onUpdateTargetTabId,
        );
    ...
    private onUpdateTargetTabId = (tabId: number): void => {
        if (
            this.state.persistedTabInfo == null ||
            this.state.persistedTabInfo.id !== tabId
        ) {
            this.updateTargetTabWithId(tabId);
        }
    };

    private onUpdateSelectedTest = (
        ...
        if (
            ...
            this.state.assessmentNavState.selectedTestType =
    ...
    private onPassUnmarkedInstances = (
            ...
            key => {
                const testStepResult: TestStepResult =
                    assessmentData.generatedAssessmentInstancesMap[key]
                        .testStepResults[payload.requirement];
                if (
                    testStepResult &&
        ...
        this.updateTestStepStatusForGeneratedInstances(
            assessmentData,
            payload.requirement,
        );
        this.emitChanged();
    ...
    private onEditFailureInstance = (
        payload: EditFailureInstancePayload,
    ): void => {
        const config = this.assessmentsProvider
            .forType(payload.test)
            .getVisualizationConfiguration();
        const assessmentData = config.getAssessmentData(this.state);
        const instances =
            assessmentData.manualTestStepResultMap[payload.requirement]
                .instances;
        for (
            let instanceIndex = 0;
    ...
    private onRemoveFailureInstance = (
        ...
        assessmentData.manualTestStepResultMap[
            payload.requirement
        ].instances = assessmentData.manualTestStepResultMap[
            payload.requirement
        ].instances.filter(instance => {
            return instance.id !== payload.id;
        });
        this.updateManualTestStepStatus(
            assessmentData,
            payload.requirement,
            payload.test,
        );
    ...
    private onAddFailureInstance = (
        payload: AddFailureInstancePayload,
    ): void => {
        const config = this.assessmentsProvider
            .forType(payload.test)
            .getVisualizationConfiguration();
    ...
    private onAddResultDescription = (
        ...
        forEach(assessmentDataMap, val => {
            ...
            if (stepResult != null) {
                stepResult.isVisualizationEnabled =
                    payload.isVisualizationEnabled;
            }
        });
    ...
    private onUndoStepStatusChange = (
        payload: ChangeRequirementStatusPayload,
    ): void => {
        const config = this.assessmentsProvider
            .forType(payload.test)
    ...
    private onChangeAssessmentVisualizationState = (
        ...
        const stepResult: TestStepResult =
            assessmentData.generatedAssessmentInstancesMap[payload.selector]
                .testStepResults[payload.requirement];
    ...
    private onUndoInstanceStatusChange = (
        payload: AssessmentActionInstancePayload,
    ): void => {
        const config = this.assessmentsProvider
            .forType(payload.test)
    ...
    private onChangeInstanceStatus = (
    ...
    private onSelectTestStep = (payload: SelectRequirementPayload): void => {
        ...
        this.state.assessmentNavState.selectedTestStep =
            payload.selectedRequirement;
        this.emitChanged();
    };
    ...
    private updateTestStepStatusOnScanUpdate(
        ...
        const isManual = this.assessmentsProvider.getStep(
            testType,
            testStepName,
        ).isManual;
        if (isManual !== true) {
    ...
    private getGroupResult(
        ...
        for (
            let keyIndex = 0;
            keyIndex < Object.keys(instanceMap).length;
            keyIndex++
        ) {
            const key = Object.keys(instanceMap)[keyIndex];
            const testStepResult: TestStepResult =
                instanceMap[key].testStepResults[testStepName];
    ...
    private updateTestStepStatusForGeneratedInstances(
        ...
        const groupResult: ManualTestStatus = this.getGroupResult(
            instanceMap,
            testStepName,
        );
        assessmentData.testStepStatus[
            testStepName
        ].stepFinalResult = groupResult;
    }
    ...
    private updateManualTestStepStatus(
        ...
        const manualResult =
            assessmentData.manualTestStepResultMap[testStepName];
        const testStepStatus = assessmentData.testStepStatus[testStepName];

        if (manualResult.status === ManualTestStatus.FAIL) {
            const hasFailureInstances: boolean = !isEmpty(
                manualResult.instances,
            );
