...
export class InitialAssessmentStoreDataGenerator {
    ...
    public generateInitialState(
        ...
        const selectedTestStep =
            ...
            first.requirements[0].key;
        const resultDescription =
            (persistedData && persistedData.resultDescription) || '';
    ...
    private constructInitialDataForAssessment(
        ...
        this.tests.forEach(test => {
            const persistedTestData =
                persistedTests && persistedTests[test.key];
            assessmentData[test.key] = test.initialDataCreator(
                test,
                persistedTestData,
            );
        });
