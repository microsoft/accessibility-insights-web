...
export class AutomatedChecksViewController extends ViewController {
    ...
    public async queryRuleGroups(): Promise<any[]> {
        return this.client.$$(
            AutomatedChecksViewSelectors.collapsibleRuleDetailsGroup,
        );
    }

    public async queryRuleGroupContents(): Promise<any[]> {
        return this.client.$$(
            AutomatedChecksViewSelectors.collapsibleContainerContent,
        );
    }

    public async toggleRuleGroupAtPosition(position: number): Promise<void> {
        await this.client.click(
            AutomatedChecksViewSelectors.getCollapseExpandButtonByGroupPosition(
                position,
            ),
        );
    }
    ...
    public async waitForVisible(): Promise<void> {
        await this.waitForSelectorVisible(
            AutomatedChecksViewSelectors.mainContainer,
        );
    }

    public async waitForScreenshotViewVisible(): Promise<void> {
        await this.waitForSelectorVisible(
            ScreenshotViewSelectors.screenshotView,
        );
    }
}
