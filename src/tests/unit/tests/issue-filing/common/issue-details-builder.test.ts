...
describe('Name of the group', () => {
    ...
    beforeEach(() => {
        markupMock = Mock.ofType<MarkupFormatter>(
            undefined,
            MockBehavior.Strict,
        );
        ...
        markupMock
            .setup(factory => factory.link(It.isAnyString()))
            .returns(href => `l-${href}-l`);
        markupMock
        ...
        markupMock
            .setup(factory => factory.sectionHeaderSeparator())
            .returns(() => '--s-h-s--');
        markupMock
            .setup(factory => factory.footerSeparator())
            .returns(() => '--f-s--');
        markupMock
            .setup(factory => factory.sectionSeparator())
            .returns(() => '\n');
    });

    it.each(['pageUrl', null])(
        'build issue details when targetApp.url is %s',
        targetAppUrl => {
