...
describe('SelectorInputListTest', () => {
    ...
    test('add selector with no space after semicolon', () => {
        ...
        addSelectorMock
            .setup(add =>
                add(
                    It.isAny(),
                    It.isValue(givenInput),
                    It.isValue(parsedSelector),
                ),
            )
    ...
    test('add selector with multiple separators', () => {
        ...
        const givenSelector =
            'iframe;selector;selectorAgain  ;    lastSelector';
    ...
    test('componentDidUpdate updates state when props have changed', () => {
        ...
        const result = Enzyme.shallow(<TestableSelectorInputList {...props} />);
        const firstState = (result.instance() as TestableSelectorInputList)
            .state.isTextFieldValueValid;
        (result.instance() as TestableSelectorInputList).setTextFieldValue(
            givenSelector,
        );
        (result.instance() as TestableSelectorInputList).componentDidUpdate(
            previousProps,
        );
        const secondState = (result.instance() as TestableSelectorInputList)
            .state.isTextFieldValueValid;
    ...
    function genericDeleteSelectorTests(
        ...
        const createdRow = Enzyme.shallow(
            itemList
                .getElement()
                .props.onRenderCell(itemList.getElement().props.items[0]),
        );
