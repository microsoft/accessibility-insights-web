...
describe('TabControllerTest', () => {
    ...
    function setupCreateTabContextMock(broadCastDelegate: () => void, tabContext, tabId: number): void {
        tabContextFactoryMock
            .setup(factory => factory.createTabContext(
                broadCastDelegate,
    ...
    test('register onConnect listener', () => {
            mockChromeAdapter.setup(mca => mca.addListenerOnConnect(It.isAny())).callback(cb => {
            }).verifiable(Times.once());
    ...
    test('onTabLoad: for main frame', () => {
            ...
            mockBroadcasterStrictMock
                .setup(md => md.getBroadcastMessageDelegate(tabId)).returns(() => broadcastDelegate)
                .verifiable(Times.once());
            ...
            mockChromeAdapter.setup(mca => mca.getTab(It.isValue(tabId), It.isAny())).returns((id, cb) => {
                getTabCallback = cb;
            }).verifiable(Times.once());
    ...
    test('onTabLoad: for child frame', () => {
            ...
            mockChromeAdapter.setup(mca => mca.addListenerOnWindowsFocusChanged(It.isAny())).callback(cb => {
                tabUpdatedCallback = cb;
            });
    ...
    test('createTabContextAndTriggerTabUpdateForExistingTabs', () => {
            ...
            mockChromeAdapter.setup(mca => mca.tabsQuery(It.isValue({}), It.isAny())).callback((query, callback) => {
    ...
    test('tab change test', () => {
            ...
            mockChromeAdapter
                .setup(mca => mca.getTab(It.isValue(tabId), It.isAny())).returns((id, cb) => {
                    getTabCallback = cb;
                })
                .verifiable(Times.once());
