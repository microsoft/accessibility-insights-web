...
describe('DeviceConnectActionCreator', () => {
    ...
    beforeEach(() => {
        ...
        deviceActionsMock
            .setup(actions => actions.connecting)
            .returns(() => connectingMock.object);
    ...
    it('validates port, connections succeeds', async () => {
        ...
        fetchScanResultsMock
            ...
            .returns(() =>
                Promise.resolve({ deviceName, appIdentifier } as ScanResults),
            );

        const connectionSucceedMock = Mock.ofType<
            Action<ConnectedDevicePayload>
        >();
        ...
        connectingMock.verify(
            connecting => connecting.invoke({ port }),
            Times.once(),
        );
        connectionSucceedMock.verify(
            succeed =>
                succeed.invoke({
                    connectedDevice: `${deviceName} - ${appIdentifier}`,
                }),
            Times.once(),
        );
    });
    ...
    it('resets to default', () => {
        ...
        resetConnectionMock.verify(
            resetConnection => resetConnection.invoke(null),
            Times.once(),
        );
    });
});
