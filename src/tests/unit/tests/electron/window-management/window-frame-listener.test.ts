...
describe(WindowFrameListener, () => {
    ...
    it('do nothing on action invocation before initialize', () => {
        browserWindowMock
            .setup(b => b.on(It.isAny(), It.isAny()))
    ...
    describe('validate window listeners', () => {
        ...
        beforeEach(() => {
            setupVerifiableWindowEventCallback(
                'maximize',
                cb => (maximizeCallback = cb),
            );
            setupVerifiableWindowEventCallback(
                'unmaximize',
                cb => (unmaximizeCallback = cb),
            );
            setupVerifiableWindowEventCallback(
                'enter-full-screen',
        ...
        it('validate window state on maximize', () => {
            windowStateActionsCreatorMock
                .setup(b =>
                    b.setWindowState({ currentWindowState: 'maximized' }),
                )
        ...
        it('validate window state on unmaximize', () => {
            windowStateActionsCreatorMock
                .setup(b =>
                    b.setWindowState({ currentWindowState: 'customSize' }),
                )
                .verifiable(Times.once());
        ...
        it('validate window state on fullscreen', () => {
            windowStateActionsCreatorMock
                .setup(b =>
                    b.setWindowState({ currentWindowState: 'fullScreen' }),
                )
                .verifiable(Times.once());
