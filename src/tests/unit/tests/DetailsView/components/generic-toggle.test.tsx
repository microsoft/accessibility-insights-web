...
import {
    GenericToggle,
    GenericToggleProps,
} from '../../../../../DetailsView/components/generic-toggle';

describe('GenericToggleTest', () => {
    let onClickMock: IMock<(
        id: string,
        enabled: boolean,
    ...
    test.each([true, false])(
        'render with string content - toggleState : %s',
        (toggleState: boolean) => {
    ...
    test('verify onclick call', () => {
        ...
        onClickMock.verify(
            ocm => ocm(props.id, !props.enabled, eventStub),
            Times.once(),
        );
    });
