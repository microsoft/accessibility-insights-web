...
describe('ScannerUtilsTest', () => {
    ...
    test('scan', (completeSignal: () => void) => {
        ...
        const expectedAxeResults: ScanResults = {
            passes: [
                getSampleRule('rule1', [selectors[0]]),
    ...
    function getAxeNodeResult(
        ...
        const axeNodeResult = getSampleNodeResultForSelector(
            selector,
            includeSnippet,
        );
    ...
    function verifyElementSelector(
        ...
        expect(ruleIds.sort()).toEqual(
            Object.keys(elementResult.ruleResults).sort(),
        );
        ...
        ruleResultIds.forEach(ruleId => {
            expect(ruleResults[ruleId].status).toBe(
                elementResult.ruleResults[ruleId].status,
            );
        });
    }
    ...
    function getSampleRule(
        ...
        selectors.forEach(selector => {
            nodes.push(
                getSampleNodeResultForSelector(selector, includeSnippet),
            );
        });
    ...
    function getSampleNodeResultForSelector(
        selector,
        includeSnippet?: boolean,
    ): AxeNodeResult {
