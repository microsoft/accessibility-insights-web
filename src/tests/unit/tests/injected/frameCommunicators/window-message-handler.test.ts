...
describe('WindowMessageHandlerTests', () => {
    ...
    beforeEach(() => {
        ...
        mockMessageMarshaller = Mock.ofType<WindowMessageMarshaller>(
            null,
            MockBehavior.Strict,
        );
    ...
    test('shouldNotInitializeMoreThanOnce', () => {
        mockWindowUtils
            .setup(x =>
                x.addEventListener(window, 'message', It.isAny(), false),
            )
            .verifiable(Times.once());
    ...
    test('verifyResponderIsCreatedForTargetWindow', () => {
        ...
        createFrameResponderCallbackMock
            .setup(x =>
                x(targetWindow, sampleMessage.command, sampleMessage.messageId),
            )
            .returns(() => () => {});
    ...
    test('shouldNotifyTargetWindowOnException', () => {
        ...
        mockWindowUtils
            .setup(x =>
                x.postMessage(
                    responseEvent.source as Window,
                    responseMessage,
                    '*',
                ),
            )
