...
# SFI ES-4.2.4 (CFS): optional Azure Artifacts feed for package restore. Left unset for
# local/GitHub builds so they keep using the default public registry; the ADO pipelines
# pass these so restore stays inside the feed that CFSClean allows. Distinct ARG names are
# used so an unset value never blanks out Yarn's own YARN_NPM_* settings.
